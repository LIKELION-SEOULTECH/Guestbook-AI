...
cuda_index = 0 # CUDA GPU SET
torch.cuda.set_device(cuda_index)
print(f"DEVICE: {device} {torch.cuda.current_device() if torch.cuda.is_available() else ''}")
...
data = pd.read_csv("/home/geonhui/MNIST_Train/5차년도_2차.csv", encoding='cp949')
...
class BERTClassifier(nn.Module):
    ...
    def gen_attention_mask(self, token_ids, valid_length):
        ...
        for i, v in enumerate(valid_length.tolist()):
...
# 더미 입력 생성 (KoBERT 입력 형식에 맞춰야 함)
dummy_input = (
    torch.randint(0, len(vocab), (1, max_len)).to(device),  # token_ids
    torch.tensor([max_len]).to(device),  # valid_length
    torch.zeros(1, max_len).long().to(device)  # segment_ids
)
...
torch.onnx.export(
    ...
    output_names=["output"],
    dynamic_axes={"token_ids": {0: "batch_size"}, "segment_ids": {0: "batch_size"}, "output": {0: "batch_size"}}
)
